<div>
    ...
    {!! Form::open(['wire:submit.prevent' => 'store']) !!}
			...
			<div id="private-office-div" class="{{$size['private_offices']['types_for_listing'] > 0 ? '':'d-none'}}">
				...
				<div class="row register-form ml-0 py-4 office-capacity-view" id="office-capacity-view">
					@if(!empty($size['private_offices']['types_for_listing']) && $size['private_offices']['types_for_listing'] > 0)
						...
						@if(!empty($size['private_offices']['types']))
						@foreach($size['private_offices']['types'] as $i => $privateOffice)
							<div class="col-12 mb-3">
								<div class="d-flex align-items-center">
									<div class="w-50">
										<input type="text" name="privateOfficeName_{{$i}}" class="size-percent common-field-input w-100 privateOfficeName" placeholder="Private Office Type {{ $i+1 }} Name" value="{{$privateOffice['name']}}" wire:model="size.private_offices.types.{{ $i }}.name"/>
									</div>
									<div class="w-50 ml-2">
										<input type="text" name="privateOfficeNumber_{{$i}}" class="size-percent common-field-input ml-2 w-100 number privateOfficeCapacity" placeholder="Number of Private Offices for Type {{ $i+1 }}" value="{{$privateOffice['quantity']}}" wire:model="size.private_offices.types.{{ $i }}.quantity"/>
									</div>
									<div class="w-50 ml-2">
										<input type="text" name="privateOfficeCapacity_{{$i}}" class="size-percent common-field-input ml-2 w-100 number privateOfficeCapacity" placeholder="Private Office Type {{ $i+1 }} Capacity" value="{{$privateOffice['capacity']}}" wire:model="size.private_offices.types.{{ $i }}.capacity"/>
									</div>
								</div>
							</div>
						{{-- @endfor --}}
			...
			<div id="meeting-room-div" class="{{$size['meeting_rooms']['types_for_listing'] > 0 ? '':'d-none'}}">
				...
				<div class="row register-form ml-1 py-4 meeting-room-view" id="meeting-room-capacity-view">
					@if(!empty($size['meeting_rooms']['types_for_listing']) && $size['meeting_rooms']['types_for_listing'] > 0)
						...
						@foreach($size['meeting_rooms']['types'] as $i => $privateOffice)
							<div class="col-12 mt-2">
								<div class="d-flex align-items-center">
									<div class="w-50">
										<input type="text" name="meetingRoomName_{{$i}}" class="size-percent common-field-input ml-2 w-100 meetingRoomName" placeholder="Meeting Room Type {{ $i+1 }} Name" value="{{$size['meeting_rooms']['types'][$i]['name']}}" wire:model="size.meeting_rooms.types.{{ $i }}.name"/>
									</div>
									<div class="w-50 ml-2">
										<input type="text" name="meetingRoomCapacity_{{$i}}" class="size-percent common-field-input ml-2 w-100 number meetingRoomCapacity" placeholder="Meeting Room Capacity {{ $i+1 }}" value="{{$size['meeting_rooms']['types'][$i]['quantity']}}" wire:model="size.meeting_rooms.types.{{ $i }}.quantity"/>
									</div>
									<div class="w-50 ml-2">
										<input type="text" name="meetingRoomCapacity_{{$i}}" class="size-percent common-field-input ml-2 w-100 number meetingRoomCapacity" placeholder="Meeting Room Capacity {{ $i+1 }}" value="{{$size['meeting_rooms']['types'][$i]['capacity']}}" wire:model="size.meeting_rooms.types.{{ $i }}.capacity"/>
									</div>
								</div>
							</div>
						@endforeach
		...
		<div class="d-flex mt-xl-0 mt-lg-5 pt-xl-5 pt-5">			
			<button type="button" wire:click="store('save')" class="btn btn-success n-bold f-9 rounded-0 mr-2 w-100" name="save" value="1">SAVE</button>
			<button type="button" wire:click="store('saveAndSubmit')" class="btn btn-success n-bold f-9 rounded-0 ml-2 w-100" name="saveAndSubmit" value="1">SAVE & SUBMIT</button>	
		</div>
	{!!Form::close()!!}
</div>
