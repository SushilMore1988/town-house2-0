    ...
    {!! Form::open(['wire:submit.prevent' => 'store']) !!}
		<div class="row px-3">
			<div class="col-md-7">
				...
				{!! Form::select('currency', ['USD' => 'USD', 'INR' => 'INR', 'SGD' => 'SGD', 'EUR' => 'EUR', 'GBP' => 'GBP', 'AUD' => 'AUD', 'RUB' => 'RUB', 'JPY' => 'JPY', 'CAD' => 'CAD'], $size['currency'], ['class' => 'form-control w-100 currency-percent', 'wire:model' => 'size.currency']) !!}
		...
		@endif
		@if(!empty($size['private_offices']['private_offices']) && $size['private_offices']['types_for_listing'] > 0)
		@foreach($size['private_offices']['private_offices'] as $private_office_number => $private_office)
		<hr/>
		<div class="row px-3">
			<div class="col-md-12">
				<h6 class="r-boldItalic f-9 check-label">Pricing : Private Office Type {{$private_office_number}} ({{$private_office['name']}})</h6>
				<div class="row">
					@foreach($size['private_offices']['private_offices'][$private_office_number]['pricing'] as $key => $value)
						@php
							if($price_setting == 'percentage'){
								$private_price = ($size['private_offices']['private_offices'][$private_office_number]['pricing'][$key]) - (($size['private_offices']['private_offices'][$private_office_number]['pricing'][$key] * $price)/100);
							}else{
								$private_price = $size['private_offices']['private_offices'][$private_office_number]['pricing'][$key] - $price;
							}
						@endphp
						<div class="col-md-2">{{$key}} :</div>
						<div class="col-md-5 col-sm-8">
							<input type="text" class="form-control number currency-percent commonClass" name="private_office_{{$private_office_number}}_{{str_replace(' ','_',$key)}}" wire:model="size.private_offices.types.{{ $private_office_number }}.pricing.{{ $key }}" value="{{ $size['private_offices']['private_offices'][$private_office_number]['pricing'][$key] }}">
						</div>
		...
		<div class="row px-3">
			<div class="col-md-12">
				<h6 class="r-boldItalic f-9 check-label">Pricing : Meeting Room {{$meeting_room_number}} ({{$meeting_room['name']}} )</h6>
				<div class="row">
					@foreach($size['meeting_rooms']['types'][$meeting_room_number]['pricing'] as $key => $value)
						...
						<div class="col-8">
							<input type="text" class="form-control number currency-percent commonClass" name="meeting_room_{{$meeting_room_number}}_{{str_replace(' ','_',$key)}}" value="{{$size['meeting_rooms']['types'][$meeting_room_number]['pricing'][$key]}}">
							<input type="text" class="form-control number currency-percent commonClass"  value="{{$size['meeting_rooms']['types'][$meeting_room_number]['pricing'][$key]}}">
						</div> --}}
					@endforeach
				</div>
			</div>
