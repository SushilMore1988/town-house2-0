...
class PrivateOfficePhotoController extends BaseController
{
    public function store(Cws $cws, Request $request){
        ...
        if($request->image){
            ...
            array_push($imageArray['private_office'], $fileName);
        } 
        $cws->images = $imageArray;
        $cws->save();
        return response()->json(['success' => 'true', 'status' => 'success', 'private_office' => $cws->images['private_office']]);
    }
}
